...
//pag. 173, @Radu Fechet, la NAD Flash se scrie si se citeste pe pagini (blocuri), nu individual !!!
...
printf("\r\nScriere in NAND Flash: \r\n");
for(j = 0; j < 2048; j++) //
printf("%x ",TxBuffer[j]);
/* Read back the written data */
FMC_NAND_ReadSmallPage(RxBuffer, WriteReadAddr, 1);
printf("\r\n\r\nCitire din NAND Flash: \r\n");
for(j = 0; j < 2048; j++) //
printf("%x ",RxBuffer[j]);	
// end of pag. 174, LAB_12
}
//
